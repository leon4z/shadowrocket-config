...
# 3. 自建组（按路由模式生成）
# --------------------------------------------------------------------------- #
# 个人版速度、国家组来自采样清单；个人稳定版只使用固定稳定组。
# 通用版沿用上游地区关键词。
# 速度组只在默认代理出口为速度时生成；稳定组只在 AI/谷歌强制走稳定时生成。
# select 不含这两个组，hybrid 和 stable 只含稳定，fallback 同时包含速度与稳定。
#
...
# v1 清单兼容性与通用版上游锚点仍使用上面的集合；v2 个人版不强求地区齐全。
PERSONAL_REQUIRED_SAMPLED_GROUPS = {"速度"}

# --------------------------------------------------------------------------- #
# 变体
# --------------------------------------------------------------------------- #
# 通用版三种路由方式，个人版另有只使用稳定组的模式。
